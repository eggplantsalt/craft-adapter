...
### 修改文件
- 📝 `README.md`
  - **改动**: 同步训练命令为当前本地模型 + torchrun 用法
  - **改动**: 增加 `num_steps_before_decay`、`use_wandb`、`console_log_freq` 参数说明
  - **改动**: 同步 CRaFT 指标说明（新增 `Lambda Before/After`）与终端日志格式

- 📝 `docs/EXPERIMENTS_AND_TRAINING.md`
  - **改动**: 更新 Baseline/CRaFT 命令为当前仓库实际可运行参数
  - **改动**: 补充 `use_wandb` 开关和 `console_log_freq` 说明
  - **改动**: 更新终端日志示例为 `Step ... | Ret ... | λ before->after ...`
  - **改动**: 明确 RLDS 训练为 step-driven（`max_steps` 终止）
